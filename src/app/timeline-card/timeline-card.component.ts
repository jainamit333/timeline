import {
  Component, ElementRef, ViewChild, Input, ViewChildren, QueryList, AfterViewInit,
  animate, transition, style, trigger, keyframes, state, OnInit, OnChanges, AfterViewChecked
} from '@angular/core';
...
export class TimelineCardComponent implements OnInit, AfterViewInit, AfterViewChecked , OnChanges{
  ...
  @Input() dateFormat: string = 'd MMM yy';
  ...
  onEventClick(event, selectedItem: TimelineElement) {
    ...
    TimelineCardComponent.updateFilling(element, this.fillingLine, this.timelineTotWidth);
  }

  initTimeline(timeLines: TimelineElement[]) {
    console.log('step3')
    let eventsMinLapse = TimelineCardComponent.minLapse(timeLines);
  ...
  updateSlide(timelineTotWidth, string) {
    ...
    let translateValue = TimelineCardComponent.getTranslateValue(this.eventsWrapper.nativeElement);
  ...
  updateTimelinePosition(string, element) {
    ...
    let timelineTranslate = TimelineCardComponent.getTranslateValue(this.eventsWrapper.nativeElement);
  ...
  translateTimeline(value: number, totWidth: number) {
    ...
    TimelineCardComponent.setTransformValue(this.eventsWrapper.nativeElement, 'translateX', value + 'px');
  ...
  private static updateFilling(selectedEvent, filling, totWidth) {
    ...
    TimelineCardComponent.setTransformValue(filling.nativeElement, 'scaleX', scaleValue);
  }
  ...
  private setDatePosition(elements: TimelineElement[], min: number, eventsMinLapse: number) {
    ...
    for (let component of elements) {
      let distance = TimelineCardComponent.dayDiff(elements[0].date, component.date);
      ...
      let aWidth = TimelineCardComponent.getElementWidth(timelineEventsArray[i].nativeElement);
      let spanWidth = TimelineCardComponent.getElementWidth(span);
  ...
  setTimelineWidth(elements: TimelineElement[], width, eventsMinLapse: number) {
    let timeSpan = TimelineCardComponent.dayDiff(elements[0].date, elements[elements.length - 1].date);
    ...
    TimelineCardComponent.updateFilling(aHref, this.fillingLine, totalWidth);
  ...
  private static minLapse(elements: TimelineElement[]) {
    ...
    for (let i = 1; i < elements.length; i++) {
      let distance = TimelineCardComponent.dayDiff(elements[i - 1].date, elements[i].date);
